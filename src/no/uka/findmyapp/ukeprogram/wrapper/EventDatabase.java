	package no.uka.findmyapp.ukeprogram.wrapper;
...
public class EventDatabase {
	...
	public ArrayList<UkaEvent> getAllEvents(ContentResolver contentResolver){
		if (Constants.DEBUG) Log.v(debug, "Inside getAllEvents");
		UkaEvent ukaEvent;
		...
		if (Constants.DEBUG) Log.v(debug , "getAllEvents cursor toString " + cursor.toString());
		cursor.moveToFirst();
		while (cursor.moveToNext()) {	
			ukaEvent = getEventFromCursor(cursor);
			eventList.add(ukaEvent);
			cursor.moveToNext();	
			if (Constants.DEBUG) Log.v(debug, "Printing from While: " + ukaEvent.toString());
		}
		if (Constants.DEBUG) Log.v(debug, "Done inserting: " + eventList.toString());
		return eventList;
	}
	...
	public ArrayList<UkaEvent> getEventsInPeriod(ContentResolver contentResolver, Timestamp fromTime, Timestamp toTime){
		UkaEvent ukaEvent;
		ArrayList<UkaEvent> eventList = new ArrayList<UkaEvent>();
		if (Constants.DEBUG) Log.v(debug, "Timestamp = " + fromTime + " to " + toTime);
		...
		if(cursor != null){
			if (Constants.DEBUG) Log.v(debug, "Cursor = " + cursor);
			cursor.moveToFirst();
			while (cursor.moveToNext()) {	
				ukaEvent = getEventFromCursor(cursor);
				eventList.add(ukaEvent);
				cursor.moveToNext();	
				if (Constants.DEBUG) Log.v("EventDatabase", "Printing from While: " + ukaEvent.toString());
			}
		}
		return eventList;
	}
	...
	public ArrayList<UkaEvent> getNextEventsFromCategory(ContentResolver contentResolver, int numberOfEvents, String eventType){
		if (Constants.DEBUG) Log.v(debug, "Inside getNextEventsFromCategory");
		UkaEvent ukaEvent;
		Date now = new Date();
		ArrayList<UkaEvent> eventList = new ArrayList<UkaEvent>();
		Cursor cursor = contentResolver.query(UkaEventContract.EVENT_CONTENT_URI, null, 		
		...
		while (cursor != null) {	
			ukaEvent = getEventFromCursor(cursor);
			if (Constants.DEBUG) Log.v(debug, cursor.toString());
			eventList.add(ukaEvent);
			cursor.moveToNext();	
		}
	...
	public UkaEvent getEventFromCursor(Cursor cursor) {
		if (Constants.DEBUG) Log.v(debug, "Inside getEventFromCursor");
	...
	public void clearEventTable(ContentResolver cr){
		if (Constants.DEBUG) Log.v(debug, "clearEventTable " + cr.toString());

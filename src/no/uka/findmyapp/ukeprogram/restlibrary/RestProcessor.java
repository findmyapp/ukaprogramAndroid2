...
import no.uka.findmyapp.ukeprogram.providers.ContentHelper;
import no.uka.findmyapp.ukeprogram.providers.UkaEvents.UkaEventContract;
import no.uka.findmyapp.ukeprogram.restlibrary.RestMethod.HTTPStatusException;
...
public class RestProcessor {
	...
	public RestProcessor(Context context, Credentials credentials) {
		if (Constants.DEBUG) Log.v(debug, "Inside RestProcessor creator");
	...
	public void callRest(ServiceModel serviceModel, String userToken) {
		if (Constants.DEBUG) Log.v(debug, "Inside callRest");
		...
		try {
			...
			if (Constants.DEBUG) Log.v(debug, "Statusexception: " + e.getMessage());
	...
	private void saveAndReturnData(ServiceModel serviceModel,
			Serializable returnedObject) {
		if (Constants.DEBUG) Log.v(debug, "saveAndReturnData");
	...
	private void prepareAndSendToContentProvider(ServiceModel serviceModel,
			Serializable returnedObject) {
		if (Constants.DEBUG) Log.v(debug, "prepare and send to content provider");
	...
	private Serializable executeAndParse(ServiceModel serviceModel, String userToken) 
		...
		try {
			...
			// Probably Time out
			if (Constants.DEBUG) Log.e(debug, "Shouldn't happen");
		}
		...
		if (Constants.DEBUG) Log.d(debug, "Response from request received: " + response);
	...
	private String httpRequest(ServiceModel serviceModel)
			...
			try {
				...
				if (Constants.DEBUG) Log.v(debug, "executeAndParse: Exception " + e.getMessage());
				throw e; 
			}
		return response;
	}
	...
	private Serializable parseFromJson(String response, Type t1)
		...
		if(response.substring(0,1).equals("[")) {
			...
			if (Constants.DEBUG) Log.v(debug, "executeAndParse: Is not list");
			s = (Serializable)mGson.fromJson(response, t1);
			if (Constants.DEBUG) Log.v(debug, "executeAndParse: Serializable " + s.toString());
		}
		return s;
	}
	...
	private Serializable parseListFromJson(String response, Type t1)
			throws JSONException {
		Serializable s;
		if (Constants.DEBUG) Log.v(debug, "executeAndParse: Is list");
	...
	private boolean isInstanceOfJavaNativeType(Serializable object) {
		if (Constants.DEBUG) Log.v(debug, "Type of " + object.toString());
		if(object instanceof String || object instanceof Integer) {
			return true;
		}
		if (Constants.DEBUG) Log.v(debug, "isInstanceOfJavaNativeType: false");
		return false; 
	}
	...
	private void sendIntentBroadcast(String intentString, Serializable obj) {
		if (Constants.DEBUG) Log.v(debug, "sendIntentBroadcast: sending broadcast, object name " 
				+ obj.getClass().getName());
		
		if (Constants.DEBUG) Log.v(debug, "sendIntentBroadcast: broadcast sent, extradata identifier: " 
				+ IntentMessages.BROADCAST_RETURN_VALUE_NAME);
		
		if (Constants.DEBUG) Log.v(debug, "sendIntentBroadcast: Putting extra " + obj.toString());
		
		if (Constants.DEBUG) Log.v(debug, "sendIntentBroadcast: intentstring: " + intentString);
